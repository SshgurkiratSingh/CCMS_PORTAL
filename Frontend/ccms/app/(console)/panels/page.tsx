    ...
    <section className="space-y-6">
      <PageHeader
        ...
        action={
          <Button
            variant="primary"
            onPress={() => router.push("/manage-panel")}
      ...
      {!loading && !filteredAndSorted.length && (
        <EmptyState
          ...
          message="No nodes match current filters."
        />
      )}
            ...
            <thead className="bg-slate-900/80 text-left text-slate-400 border-b border-slate-700">
              <tr>
                {[
                  ...
                  <th
                    key={h}
                    className={`px-4 py-3 font-medium ${h === "" ? "text-right" : ""}`}
                  >
                    {h}
                  </th>
                ))}
