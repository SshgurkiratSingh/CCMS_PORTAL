...
void updateDashboard() {
  Serial.print("\033[H");
  Serial.println("================ REAL-TIME METER DASHBOARD ================");
  int columns = 0;

  for (uint16_t i = startAddr; i <= endAddr; i += 2) {
    uint8_t result = node.readHoldingRegisters(i, 2);
    delay(120);
    ...
    if (result == node.ku8MBSuccess) {
      uint16_t r1 = node.getResponseBuffer(0);
      uint16_t r2 = node.getResponseBuffer(1);
      float val = decodeFloatFromRegisters(r1, r2, useByteSwap);
  ...
  Serial.println("\n--- MQTT Target Registers ---");
  for (int i = 0; i < numMqttRegs; i++) {
    Serial.print("R");
    Serial.print(mqttRegisters[i]);
    Serial.print(": ");
    Serial.println(mqttValues[i], 2);
  }
  Serial.println("-----------------------------------------------------------");
}
