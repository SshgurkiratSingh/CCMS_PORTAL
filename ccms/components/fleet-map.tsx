...
export default function FleetMap({
  ...
  return (
    <div
      className={className || "h-[600px] w-full rounded-xl overflow-hidden border border-slate-700 shadow-xl relative z-0"}
    >
      <MapContainer
            ...
            <Popup className="custom-popup">
              <div className="font-sans text-slate-800 p-1">
                ...
                <p className="text-xs mb-1">
                  Status:{" "}
                  <span
                    className={`font-semibold ${panel.status === "ONLINE" ? "text-emerald-600" : panel.status === "FAULT" ? "text-rose-600" : "text-slate-500"}`}
                  >
                    {panel.status}
                  </span>
                </p>
