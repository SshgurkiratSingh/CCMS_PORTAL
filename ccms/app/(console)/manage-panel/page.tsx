  ...
  return (
    <section className="max-w-2xl mx-auto space-y-6 pb-12">
      <div className="flex flex-col gap-2 border-b border-slate-800 pb-4">
        <Link href="/panels" className="inline-flex items-center gap-2 text-sm text-cyan-400 hover:text-cyan-300 transition-colors mb-2 font-medium">
          <ArrowLeft className="h-4 w-4" /> Back to Fleet
        </Link>
        <h2 className="text-3xl font-bold tracking-tight text-slate-100 flex items-center gap-2">
          {editId ? <Save className="h-6 w-6 text-indigo-400" /> : <PlusCircle className="h-6 w-6 text-emerald-400" />}
          {editId ? "Update Node Configuration" : "Provision New Node"}
        </h2>
        <p className="text-sm text-slate-400">
          {editId ? "Modify the metadata and spatial coordinates of an existing edge node." : "Register a new edge node controller into the fleet ecosystem."}
        </p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6 space-y-4">
          <div className="space-y-1">
            <label className="text-sm font-medium text-slate-300">Node / Panel ID <span className="text-rose-400">*</span></label>
            <input 
              required
              disabled={!!editId}
              type="text" 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-300">MAC Address</label>
              <input 
                type="text" 
                value={macAddress}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6 space-y-4">
          <h3 className="font-semibold text-slate-200 mb-2">Spatial Localization</h3>
          <div className="space-y-1">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          >
            {saving ? <span className="animate-pulse">Saving...</span> : <>{editId ? 'Save Changes' : 'Provision Node'}</>}
          </button>
        </div>
      </form>
    </section>
